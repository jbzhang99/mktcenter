 ...
 */
@Component
public class SendSmsIntegralAddListener implements MessageListener {

    @Resource(name="sendSmsTemplateMessageIntegralAddServiceImpl")
    private SendSmsTemplateMessageService sendSmsTemplateMessageService;

    @Autowired
    private RocketLogService rocketLogService;

    private static final Logger logger = LoggerFactory.getLogger(SendSmsIntegralAddListener.class);

    @Override
    public Action consume(Message message, ConsumeContext consumeContext) {

        logger.info("sendSmsTemplateMessageIntegralAddServiceImpl消费消息 topic:" + message.getTopic() + " messageId:" + message.getMsgID());

//        Runnable runnable = new ThreadExtend("sendSmsIntegralAddListener", "sendSmsIngegralCustomer", message);
