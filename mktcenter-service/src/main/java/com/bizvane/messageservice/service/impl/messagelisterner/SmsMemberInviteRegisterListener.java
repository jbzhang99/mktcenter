 ...
 */
@Component
public class SmsMemberInviteRegisterListener implements MessageListener {

    @Resource(name="smsTemplateMessageMemberInviteRegisterServiceImpl")
    private SmsTemplateMessageService smsTemplateMessageService;

    @Autowired
    private RocketLogService rocketLogService;

    private static final Logger logger = LoggerFactory.getLogger(SmsMemberInviteRegisterListener.class);

    @Override
    public Action consume(Message message, ConsumeContext consumeContext) {

        logger.info("SmsTemplateMessageMemberInviteRegisterServiceImpl消费消息 topic:" + message.getTopic() + " messageId:" + message.getMsgID());

//        Runnable runnable = new ThreadExtend("smsMemberInviteRegisterListener", "sendMemberCustomer", message);
