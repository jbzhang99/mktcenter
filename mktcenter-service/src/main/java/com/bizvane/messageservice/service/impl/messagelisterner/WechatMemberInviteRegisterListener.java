 ...
 */
@Component
public class WechatMemberInviteRegisterListener implements MessageListener {

    @Resource(name="wXTemplateMessageMemberInviteRegisterServiceImpl")
    private WXTemplateMessageService wxTemplateMessageService;

    @Autowired
    private RocketLogService rocketLogService;

    private static final Logger logger = LoggerFactory.getLogger(WechatMemberInviteRegisterListener.class);

    @Override
    public Action consume(Message message, ConsumeContext consumeContext) {

        logger.info("WXTemplateMessageMemberInviteRegisterServiceImpl消费消息 topic:" + message.getTopic() + " messageId:" + message.getMsgID());

//        Runnable runnable = new ThreadExtend("wechatMemberInviteRegisterListener", "sendMemberCustomer", message);
