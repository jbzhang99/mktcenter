 ...
 */
@Component
public class WechatMemberRegisterListener implements MessageListener {

    @Resource(name="wXTemplateMessageMemberRegisterServiceImpl")
    private WXTemplateMessageService wxTemplateMessageService;

    @Autowired
    private RocketLogService rocketLogService;

    private static final Logger logger = LoggerFactory.getLogger(WechatMemberRegisterListener.class);

    @Override
    public Action consume(Message message, ConsumeContext consumeContext) {

        logger.info("WXTemplateMessageMemberRegisterServiceImpl消费消息 topic:" + message.getTopic() + " messageId:" + message.getMsgID());

//        Runnable runnable = new ThreadExtend("wechatMemberRegisterListener", "sendMemberCustomer", message);
