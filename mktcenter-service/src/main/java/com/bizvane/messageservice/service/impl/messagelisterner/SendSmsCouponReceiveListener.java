 ...
 */
@Component
public class SendSmsCouponReceiveListener implements MessageListener {

    @Resource(name="sendSmsTemplateMessageCouponServiceImpl")
    private SendSmsTemplateMessageService sendSmsTemplateMessageService;

    @Autowired
    private RocketLogService rocketLogService;

    private static final Logger logger = LoggerFactory.getLogger(SendSmsCouponReceiveListener.class);

    @Override
    public Action consume(Message message, ConsumeContext consumeContext) {

        logger.info("SendSmsTemplateMessageCouponServiceImpl消费消息 topic:" + message.getTopic() + " messageId:" + message.getMsgID());
//
//        Runnable runnable = new ThreadExtend("sendSmsCouponReceiveListener", "sendSmsCouponCustomer", message);
