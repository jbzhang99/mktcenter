 ...
 */
@Component
public class WechatCouponExpireListener implements MessageListener{

    @Resource(name="wXTemplateMessageCouponExpireServiceImpl")
    private WXTemplateMessageService wxTemplateMessageService;

    @Autowired
    private RocketLogService rocketLogService;

    private static final Logger logger = LoggerFactory.getLogger(WechatCouponExpireListener.class);

    @Override
    public Action consume(Message message, ConsumeContext consumeContext) {

        logger.info("WXTemplateMessageCouponExpireServiceImpl消费消息 topic:" + message.getTopic() + " messageId:" + message.getMsgID());

//        Runnable runnable = new ThreadExtend("wechatCouponExpireListener", "sendCouponCustomer", message);
