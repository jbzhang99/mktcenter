 ...
 */
@Component
public class SendSmsIntegralExpireListener implements MessageListener {

    @Resource(name="sendSmsTemplateMessageIntegralExpireServiceImpl")
    private SendSmsTemplateMessageService sendSmsTemplateMessageService;

    @Autowired
    private RocketLogService rocketLogService;

    private static final Logger logger = LoggerFactory.getLogger(SendSmsIntegralExpireListener.class);

    @Override
    public Action consume(Message message, ConsumeContext consumeContext) {

        logger.info("sendSmsTemplateMessageIntegralExpireServiceImpl消费消息 topic:" + message.getTopic() + " messageId:" + message.getMsgID());

//        Runnable runnable = new ThreadExtend("sendSmsIntegralExpireListener", "sendSmsIngegralCustomer", message);
