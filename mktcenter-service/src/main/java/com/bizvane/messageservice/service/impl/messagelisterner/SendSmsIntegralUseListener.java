 ...
 */
@Component
public class SendSmsIntegralUseListener implements MessageListener {

    @Resource(name="sendSmsTemplateMessageIntegralUseServiceImpl")
    private SendSmsTemplateMessageService sendSmsTemplateMessageService;

    @Autowired
    private RocketLogService rocketLogService;

    private static final Logger logger = LoggerFactory.getLogger(SendSmsIntegralUseListener.class);

    @Override
    public Action consume(Message message, ConsumeContext consumeContext) {

        logger.info("sendSmsTemplateMessageIntegralUseServiceImpl消费消息 topic:" + message.getTopic() + " messageId:" + message.getMsgID());

//        Runnable runnable = new ThreadExtend("sendSmsIntegralUseListener", "sendSmsIngegralCustomer", message);
