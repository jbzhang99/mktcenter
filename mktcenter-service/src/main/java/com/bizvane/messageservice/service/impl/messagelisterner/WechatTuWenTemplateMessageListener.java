 ...
 */
@Component
public class WechatTuWenTemplateMessageListener implements MessageListener {

    @Resource(name="wXTuWenTemplateMessageServiceImpl")
    private WXTemplateMessageService wxTemplateMessageService;

    @Autowired
    private RocketLogService rocketLogService;

    private static final Logger logger = LoggerFactory.getLogger(WechatTuWenTemplateMessageListener.class);

    @Override
    public Action consume(Message message, ConsumeContext consumeContext) {

        logger.info("WXTuWenTemplateMessageServiceImpl消费消息 topic:" + message.getTopic() + " messageId:" + message.getMsgID());
