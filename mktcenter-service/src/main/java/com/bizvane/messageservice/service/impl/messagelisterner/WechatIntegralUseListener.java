 ...
 */
@Component
public class WechatIntegralUseListener implements MessageListener{

    @Resource(name="wXTemplateMessageIntegralUseServiceImpl")
    private WXTemplateMessageService wxTemplateMessageService;

    @Autowired
    private RocketLogService rocketLogService;

    private static final Logger logger = LoggerFactory.getLogger(WechatIntegralUseListener.class);

    @Override
    public Action consume(Message message, ConsumeContext consumeContext) {

        logger.info("WXTemplateMessageIntegralUseServiceImpl消费消息 topic:" + message.getTopic() + " messageId:" + message.getMsgID());

//        Runnable runnable = new ThreadExtend("wechatIntegralUseListener", "sendIntegralCustomer", message);
