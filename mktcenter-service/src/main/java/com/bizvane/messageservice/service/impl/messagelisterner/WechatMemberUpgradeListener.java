 ...
 */
@Component
public class WechatMemberUpgradeListener implements MessageListener {

    @Resource(name="wXTemplateMessageMemberUpgradeServiceImpl")
    private WXTemplateMessageService wxTemplateMessageService;

    @Autowired
    private RocketLogService rocketLogService;

    private static final Logger logger = LoggerFactory.getLogger(WechatMemberUpgradeListener.class);

    @Override
    public Action consume(Message message, ConsumeContext consumeContext) {

        logger.info("WXTemplateMessageMemberUpgradeServiceImpl消费消息 topic:" + message.getTopic() + " messageId:" + message.getMsgID());

//        Runnable runnable = new ThreadExtend("wechatMemberUpgradeListener", "sendMemberCustomer", message);
