 ...
 */
@Component
public class WechatMemberDowngradeListener implements MessageListener {

    @Resource(name="wXTemplateMessageMemberDowngradeServiceImpl")
    private WXTemplateMessageService wxTemplateMessageService;

    @Autowired
    private RocketLogService rocketLogService;

    private static final Logger logger = LoggerFactory.getLogger(WechatMemberDowngradeListener.class);

    @Override
    public Action consume(Message message, ConsumeContext consumeContext) {

        logger.info("WXTemplateMessageMemberDowngradeServiceImpl消费消息 topic:" + message.getTopic() + " messageId:" + message.getMsgID());

//        Runnable runnable = new ThreadExtend("wechatMemberDowngradeListener", "sendMemberCustomer", message);
