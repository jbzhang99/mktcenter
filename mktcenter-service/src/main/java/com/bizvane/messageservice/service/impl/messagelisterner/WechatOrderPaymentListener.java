 ...
 */
@Component
public class WechatOrderPaymentListener implements MessageListener {

    @Resource(name="wXTemplateMessageOrderPaymentServiceImpl")
    private WXTemplateMessageService wxTemplateMessageService;

    @Autowired
    private RocketLogService rocketLogService;

    private static final Logger logger = LoggerFactory.getLogger(WechatOrderPaymentListener.class);

    @Override
    public Action consume(Message message, ConsumeContext consumeContext) {

        logger.info("WXTemplateMessageOrderPaymentServiceImpl消费消息 topic:" + message.getTopic() + " messageId:" + message.getMsgID());

//        Runnable runnable = new ThreadExtend("wechatOrderPaymentListener", "sendOrderCustomer", message);
