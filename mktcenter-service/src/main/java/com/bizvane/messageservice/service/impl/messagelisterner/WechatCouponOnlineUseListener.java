 ...
 */
@Component
public class WechatCouponOnlineUseListener implements MessageListener {

    @Resource(name="wXTemplateMessageCouponOnlineUseServiceImpl")
    private WXTemplateMessageService wxTemplateMessageService;

    @Autowired
    private RocketLogService rocketLogService;

    private static final Logger logger = LoggerFactory.getLogger(WechatCouponOnlineUseListener.class);

    @Override
    public Action consume(Message message, ConsumeContext consumeContext) {

        logger.info("WXTemplateMessageCouponOnlineUseServiceImpl消费消息 topic:" + message.getTopic() + " messageId:" + message.getMsgID());

//        Runnable runnable = new ThreadExtend("wechatCouponOnlineUseListener", "sendCouponCustomer", message);
