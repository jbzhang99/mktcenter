 ...
 */
@Component
public class WechatOrderCancelListener implements MessageListener {

    @Resource(name="wXTemplateMessageOrderCancelServiceImpl")
    private WXTemplateMessageService wxTemplateMessageService;

    @Autowired
    private RocketLogService rocketLogService;

    private static final Logger logger = LoggerFactory.getLogger(WechatOrderCancelListener.class);

    @Override
    public Action consume(Message message, ConsumeContext consumeContext) {

        logger.info("WXTemplateMessageOrderCancelServiceImpl消费消息 topic:" + message.getTopic() + " messageId:" + message.getMsgID());

//        Runnable runnable = new ThreadExtend("wechatOrderCancelListener", "sendOrderCustomer", message);
