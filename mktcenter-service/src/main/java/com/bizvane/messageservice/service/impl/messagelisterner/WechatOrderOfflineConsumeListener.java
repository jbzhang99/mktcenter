 ...
 */
@Component
public class WechatOrderOfflineConsumeListener implements MessageListener {

    @Resource(name="wXTemplateMessageOrderOfflineConsumeServiceImpl")
    private WXTemplateMessageService wxTemplateMessageService;

    @Autowired
    private RocketLogService rocketLogService;

    private static final Logger logger = LoggerFactory.getLogger(WechatOrderOfflineConsumeListener.class);

    @Override
    public Action consume(Message message, ConsumeContext consumeContext) {

        logger.info("WXTemplateMessageOrderOfflineConsumeServiceImpl消费消息 topic:" + message.getTopic() + " messageId:" + message.getMsgID());

//        Runnable runnable = new ThreadExtend("wechatOrderOfflineConsumeListener", "sendOrderCustomer", message);
