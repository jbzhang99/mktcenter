 ...
 */
@Component
public class SmsMemberUpgradeListener implements MessageListener {

    @Resource(name="smsTemplateMessageMemberUpgradeServiceImpl")
    private SmsTemplateMessageService smsTemplateMessageService;

    @Autowired
    private RocketLogService rocketLogService;

    private static final Logger logger = LoggerFactory.getLogger(SmsMemberUpgradeListener.class);

    @Override
    public Action consume(Message message, ConsumeContext consumeContext) {

        logger.info("smsTemplateMessageMemberUpgradeServiceImpl消费消息 topic:" + message.getTopic() + " messageId:" + message.getMsgID());

//        Runnable runnable = new ThreadExtend("smsMemberUpgradeListener", "sendMemberCustomer", message);
